...
public class Generator {
    ...
    private JRadioButton szyfrowanieNaEkranRadioButton;
    private JRadioButton szyfrowanieDoPlikuRadioButton;
    private JTextField SzyfratorOutputPath;
    private JTextField TextPath;
    private JTextArea SzyfratorWynik;
    ...
    public Generator(){
        ...
        Szyfruj.addActionListener(e -> {
            ...
            for(int i=0;i<testByte.length;i++){
                String bin = Integer.toBinaryString(testByte[i]);
                textCode.append(bin);
            }
            StringBuilder code = new StringBuilder();
            for(int i = 0; i < textCode.length()&&i<result.length(); i++)
                code.append((result.charAt(i) ^ textCode.toString().charAt(i)));
            String coded = code.toString();
            //int intts = Integer.parseInt(textCode.toString(),2);
            BigInteger bigInt = BigInteger.valueOf(Integer.parseInt(textCode.toString(),2));
            String str = new String(bigInt.toByteArray(), Charset.defaultCharset());
            if (szyfrowanieNaEkranRadioButton.isSelected()) {
                SzyfratorWynik.setText(str);
            } else if (szyfrowanieDoPlikuRadioButton.isSelected()) {
                PrintWriter printWriter = null;
                try {
                    printWriter = new PrintWriter(SzyfratorOutputPath.getText());
                } catch (FileNotFoundException ex) {
                    ex.printStackTrace();
                }
                assert printWriter != null;
